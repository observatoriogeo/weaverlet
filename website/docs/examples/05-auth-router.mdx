---
title: "05: Auth router"
sidebar_label: "05: Auth router"
---
...
- For `keep_mounted=True` mode, see the warning in [`AuthRouterComponent`](../routing/auth-router) about reading `flask.session` inside callbacks instead of capturing `user` from `get_layout`.
...
- [`AuthRouterComponent`: API reference](../api/auth-router)
